...
class RadioPlayer(val musicPlayer: MusicPlayer, uri: Uri) {
    var usable = false
    ...
    val fadePlayback: Boolean
        get() = musicPlayer.settings.getFadePlayback()
    private val fadePlaybackDuration: Int
        get() = (musicPlayer.settings.getFadePlaybackDuration() * 1000).toInt()
    ...
    init {
        unsafeMediaPlayer = MediaPlayer().apply {
            ...
            setDataSource(musicPlayer.applicationContext, uri)
        }
    }
    ...
    private fun creationDurationTimer() {
